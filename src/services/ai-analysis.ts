...
/**
 * Mock AI analysis service - simulates product analysis
...
/**
 * Simulate AI analysis based on image context
 * Uses filename hints and random category selection
 */
export function analyzeProduct(_imageDataUrl?: string): AIAnalysisResult {
  // Map common image names to categories
  const keywords = [
    "pottery",
    "terracotta",
    "clay",
  ...
  // Try to detect category from filename/data
  let detectedCategory = "general";
  if (_imageDataUrl) {
    const lower = _imageDataUrl.toLowerCase();
    for (const kw of keywords) {
      if (lower.includes(kw)) {
        ...
        if (["pottery", "terracotta", "clay", "vase", "pot", "ceramic"].includes(kw)) {
          ...
          ["textile", "fabric", "weave", "cloth", "cotton", "silk", "scarf", "saree"].includes(kw)
        ) {
  ...
  // If no specific match, pick randomly for variety
